...
let hints = function() {
    let info = new Alert().showMessage(
        "info",
        `<strong>Hints:</strong>
...
if (window.location.protocol === "http:") {
    let httpWarning = new Alert().showMessage(
        "warning",
        "Warning: You're using an insecure (http) connection. Some cryptographic operations may not work as expected."
    );
...
const handleFileDownload = e => {
    if (window.location.hash) {
        ...
        if (currentLink.classList.contains("download-button")) {
            //if used clicked on the download button

            if (currentLink.href.includes("/dir")) {
                //Gian: I'm only decrypting a file once...

                const filePath =
                    window.location.href.replace(location.hash, "").replace("/dir", "/cat") +
                    "/" +
                    currentLink.getAttribute("file-name");

                fetchFilePath(filePath, currentLink.getAttribute("file-name"));
            }
        } else {
            //we are dealing with file name click then
...
const createShareLink = () => {
    ...
    const bodyMessage =
        encodeURI(
            "This is your new SendCrypt secure link, be careful and don't share it with anyone you don't really, REALLY, trust:"
        ) +
        "%0D%0A" +
        "%0D%0A" +
        ...
        .then(files => {
            ...
            for (let button of buttons) {
                button.addEventListener("click", e => {
                    ...
                    let deleteItemName =
                        e.currentTarget.parentElement.parentElement.firstElementChild.children[1].innerText;
                    ...
                    if (window.confirm(`Are you sure you want to delete ${deleteItemName}`)) {
                        fetch(`${window.location.pathname}/${deleteItemName}`, {
                            ...
                            .catch(error => {
                                let deleteError = new Alert().showMessage(
                                    "danger",
                                    `Error while trying to delete your file: ${error}`
                                );
                                console.log(error);
                            });
                    }
        ...
        *-------------------------------------------------------------*/
        ...
        form.addEventListener("_submit", function(e) {
            ...
            setTimeout(() => {
                ...
                // if (form.classList.contains('is-uploading')) console.log('is-uploading');
                ...
                if (isAdvancedUpload) {
                    ...
                    const uploadFile = async ajax => {
                        ...
                        if (data.success) {
                            ...
                            let successUpload = new Alert().showMessage(
                                "success",
                                `Your file was successfully uploaded!  Remember, this link provides <strong>full access </strong>to your folder. If you email the link, it's essentially the same as emailing the original contents.`
                            );
                        }

                        document.querySelector(".box__message").innerHTML =
                            "Uploaded to your <a class='secure-link' href='/dir/" +
                            data.dir +
                            "#" +
                            hash +
                            "'> secure link </a>. <p><strong>Do not lose this link</strong>, or the uploaded files will never be found again!</p>";
...
emailLinkBtn.addEventListener("click", function(e) {
    ...
    let emailModal = new Modal().open(
        "E-mail Your Link",
        "Send your link to someone trustable",
